            ...
            <div class="flex items-center space-x-4">
                <div class="flex-grow">
                    <input type="text" name="search" id="search" value="{{ $search ?? '' }}" placeholder="Cari Produk" 
                        class="w-full px-3 py-2 bg-white text-white border border-gray-700 rounded placeholder-gray-500 focus:text-black focus:bg-white focus:outline-none transition duration-150 ease-in-out">
                </div>
                <div>
                    <select name="sort" id="sort" class="bg-gray-700 text-white font-bold py-2 px-4 rounded">
                        <option value="">Urutkan</option>
                        <option value="price_asc" {{ request('sort') == 'price_asc' ? 'selected' : '' }}>Harga: Rendah ke Tinggi</option>
                        <option value="price_desc" {{ request('sort') == 'price_desc' ? 'selected' : '' }}>Harga: Tinggi ke Rendah</option>
                    </select>
                </div>
                <button type="submit" id="searchButton" class="bg-red-500 hover:bg-red-700 text-white font-bold py-2 px-4 rounded">
                    Cari
                </button>
            </div>
        </form>
...
<script>
    $(document).ready(function() {
        ...
        var debounceTimer;
        $('#search, #sort').on('change keyup', function() {
